...
bool PathPlannerGrid::isBlocked(int ngr, int ngc){
  if(ngr >= 0 && ngc >=0 && ngr < rcells -1 && ngc < ccells -1 && world_grid[ngr][ngc].voronoi_id>=1)
  {
...
void PathPlannerGrid::findshortest(AprilInterfaceAndVideoCapture &testbed){
  ...
  if(goal_grid_x < 0 || goal_grid_y < 0 || goal_grid_x >= rcells-1 || goal_grid_y >= ccells -1)
  {
...
bool PathPlannerGrid::bactrackValidityForBSA_CM(pair <int, int> t, int nx, int ny, int j)
{
  ...
  {
    cout<<i<<endl;
    cout<<"blockedcellcheck[nx][ny][j][i].first, second: "<< blockedcellcheck[nx][ny][j][i].first<<" "<<blockedcellcheck[nx][ny][j][i].second<<endl; 
...
void PathPlannerGrid::defineVoronoiPartition(AprilInterfaceAndVideoCapture &testbed, vector<PathPlannerGrid> &bots){
  ...
  for(int r = 0; r < rcells-1; r++)//declaring boundary cells to the points which are in direct contact with the other boundary
  {
  ...
  for(int r = 0; r < rcells-1; r++)//declaring boundary cells, which are in contact to the other boundary through a corner
  {
    for(int c = 0; c < ccells-1; c++)
    {
      if(world_grid[r][c].isBoundaryCell) continue;
...
void PathPlannerGrid::discoverPatch(int patch_count, int r, int c)
{
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    if(ngr[i]<0 || ngc[i]< 0 || ngr[i] >= rcells-1 || ngc[i] >= ccells-1)
      continue;
    ...
    //{
      ...
      for(int i = 0; i < 4; i++)
      {
        ...
        if(r1>=0 && c1>=0 && r1<rcells-1 && c1 < ccells -1)
        {
  ...
  for(int i = 0; i < patches.size(); i++)
  {
    ...
    for(int j = 0; j < patches[i].size(); j++)
    {
      ...
      for(int k = 0; k < 4; k++)
      {
        ...
        if(ngr < 0 || ngc < 0 || ngr >= rcells-1 || ngc >= ccells-1) continue;
...
void PathPlannerGrid::VoronoiPartitionBasedOnlineCoverage(AprilInterfaceAndVideoCapture &testbed, robot_pose &ps, double reach_distance, vector<PathPlannerGrid> &bots){
  ...
  if(!first_call){
    ...
    //checking validit of backtrackpoints made as of now.
      ...
      //checking validity of uevpoints made as of now.
      for(int i = 0; i< uev_destinations.size();i++){
       if(!uev_destinations[i].valid || world_grid[uev_destinations[i].next_p.first][uev_destinations[i].next_p.second].steps>0 || world_grid[uev_destinations[i].next_p.first][uev_destinations[i].next_p.second].voronoi_id!=robot_tag_id){//the bt is no longer uncovered or backtack conditions no longer remain
            uev_destinations[i].valid = false;//the point should no longer be considered in future
            continue;
          }
      }
     ...
     /* unvisited_discovered_boundary=0;
      ...
      for(int r = 0; r < rcells-1; r++)//checking consistency and updating the boundary cells discovered
      {
        for(int c = 0; c < ccells-1; c++)
        {
  ...
  if(current_behaviour == 0){//is the robot is yet to reach the boundary, this behaviour is same as boustraophoden motion but the preferred direction is not necesarily up
      ...
      for(int i = 0; i < bt_destinations.size(); i++)
      {
        if(!bt_destinations[i].valid || bt_destinations[i].manhattan_distance < 0) continue;
    ...
    //if function reaches this point, it means it doesn't' have a valid bt_point, so searching amongst UEV
      ...
      for(int i = 0; i < uev_destinations.size(); i++)
      {
        if(!uev_destinations[i].valid || uev_destinations[i].manhattan_distance < 0) continue;
  ...
  {
    pair<int,int> t = sk.top();  
    int nx = t.first - world_grid[t.first][t.second].parent.first + 1;
    int ny = t.second - world_grid[t.first][t.second].parent.second + 1;
    if(first_inBoundary_call)
    {
      first_inBoundary_call = 0;
      //int ngr = t.first + aj[nx][ny][1];
      //int ngc = t.first + aj[nx][ny][1];

      vector <int> iter_order = {1, 0, 3, 2};//right, front, back, left
      start_boundary_x = t.first;
      start_boundary_y = t.second;    
      for(int i = 0; i < 4; i++)
      {
    ...
    }//if first boundary call
    
    //not a first boundary call
    //assuming there is now a boundary to left
    vector <int> iter_order = {2, 0, 1, 3};//left, front, right, back
    for(int i = 0; i < 4; i++)
    {
      int ngr = t.first + aj[nx][ny][iter_order[i]].first;
      int ngc = t.second + aj[nx][ny][iter_order[i]].second;
      if(isEmpty(ngr, ngc) && world_grid[ngr][ngc].voronoi_id==robot_tag_id && world_grid[ngr][ngc].patch_num==-1)
        {
          if(ngr == start_boundary_x && ngc==start_boundary_y)//checking if next point is starting boundary point
          {
            ...
            for(j = 0; j < 4; j++)
            {
              int ngr2 = ngr + aj[nx][ny][iter_order[j]].first;
              int ngc2 = ngc + aj[nx][ny][iter_order[j]].second;
            ...
            if(j==4)
            {
              cout<<"boundary traversal complete\n";
              current_behaviour = 2;
              return;
            }
          }//if ngr
  ...
  if(current_behaviour==2)//auctioning and repartition
  {
    ...
    for(int i = 0; i < boundary_points.size(); i++)
    {
      ...
      for(int j = 0; j < 4; j++)
      {
        ...
        if(ngr[j]<0 || ngc[j]<0 || ngr[j]>=rcells-1 || (ngc[j] >= ccells -1)) continue; 
      ...
      //cv::waitKey(0);
      //patch_count++;
      int patch_size = patches.size();
      patches.resize(patch_size+1);
      discoverPatch(patch_size, boundary_points[i].cell.first, boundary_points[i].cell.second);
      repartition(bots);
